...
public final class LocatorBarConfigScreen extends Screen {
    ...
    private pl.fuzjajadrowa.locatorbar.config.LocatorBarEnums.BarTheme selectedTheme;
    private pl.fuzjajadrowa.locatorbar.config.LocatorBarEnums.SoundPack selectedSoundPack;
    private boolean selectedSoundEnabled;
    private boolean selectedFadeAnimation;
    private boolean selectedShowMarkerDistances;
    private boolean selectedShowMarkerHeight;
    private boolean selectedShowDegrees;
    private boolean selectedShowOffscreenArrows;
    private int page = 0;
    ...
    private Button dragHudButton;
    private Button soundButton;
    private Button soundPackButton;
    private Button fadeAnimationButton;
    private Button showMarkerDistancesButton;
    private Button showMarkerHeightButton;
    private Button degreesButton;
    private Button offscreenArrowsButton;
    ...
    public LocatorBarConfigScreen(Screen parent) {
        ...
        this.selectedTheme = LocatorBarConfig.getTheme();
        this.selectedSoundPack = LocatorBarConfig.getSoundPack();
        this.selectedSoundEnabled = LocatorBarConfig.isSoundEnabled();
        this.selectedFadeAnimation = LocatorBarConfig.isFadeAnimation();
        this.selectedShowMarkerDistances = LocatorBarConfig.isShowMarkerDistances();
        this.selectedShowMarkerHeight = LocatorBarConfig.isShowMarkerHeight();
        this.selectedShowDegrees = LocatorBarConfig.isShowDegrees();
        this.selectedShowOffscreenArrows = LocatorBarConfig.isShowOffscreenArrows();
    }
    ...
    protected void init() {
        ...
        themeButton = Button.builder(themeButtonText(), button -> cycleTheme()).bounds(0, 0, 120, 20).build();
        dragHudButton = Button.builder(Component.translatable("locatorbar.config.button.drag_hud"), button -> {
            if (this.minecraft != null) {
                //? if >=26.2 {
                this.minecraft.setScreenAndShow(new LocatorBarPositionScreen(this));
                //?} else {
                /*this.minecraft.setScreen(new LocatorBarPositionScreen(this));
                *///?}
            }
        }).bounds(0, 0, 120, 20).build();
        soundButton = Button.builder(soundButtonText(), button -> toggleSound()).bounds(0, 0, 120, 20).build();
        soundPackButton = Button.builder(soundPackButtonText(), button -> cycleSoundPack()).bounds(0, 0, 120, 20).build();
        fadeAnimationButton = Button.builder(fadeAnimationButtonText(), button -> toggleFadeAnimation()).bounds(0, 0, 120, 20).build();
        showMarkerDistancesButton = Button.builder(showMarkerDistancesButtonText(), button -> toggleShowMarkerDistances()).bounds(0, 0, 120, 20).build();
        showMarkerHeightButton = Button.builder(showMarkerHeightButtonText(), button -> toggleShowMarkerHeight()).bounds(0, 0, 120, 20).build();
        degreesButton = Button.builder(degreesButtonText(), button -> toggleDegrees()).bounds(0, 0, 120, 20).build();
        offscreenArrowsButton = Button.builder(offscreenArrowsButtonText(), button -> toggleOffscreenArrows()).bounds(0, 0, 120, 20).build();
    ...
    *///?}
    ...
    private void cycleTheme() {
        ...
        themeButton.setMessage(themeButtonText());
        LocatorBarConfig.setTheme(selectedTheme);
        LocatorBarConfig.save();
    }

    private Component themeButtonText() {
        return Component.translatable(selectedTheme.translationKey());
    }

    private void toggleSound() {
        selectedSoundEnabled = !selectedSoundEnabled;
        soundButton.setMessage(soundButtonText());
        LocatorBarConfig.setSoundEnabled(selectedSoundEnabled);
        LocatorBarConfig.save();
        updateControlStates();
    }

    private Component soundButtonText() {
        return Component.translatable(selectedSoundEnabled ? "locatorbar.option.on" : "locatorbar.option.off");
    }

    private void cycleSoundPack() {
        selectedSoundPack = selectedSoundPack.next();
        soundPackButton.setMessage(soundPackButtonText());
        LocatorBarConfig.setSoundPack(selectedSoundPack);
        LocatorBarConfig.save();
    }

    private Component soundPackButtonText() {
        return Component.translatable(selectedSoundPack.translationKey());
    }

    private void toggleDegrees() {
        selectedShowDegrees = !selectedShowDegrees;
        degreesButton.setMessage(degreesButtonText());
        LocatorBarConfig.setShowDegrees(selectedShowDegrees);
        LocatorBarConfig.save();
    }

    private Component degreesButtonText() {
        return Component.translatable(selectedShowDegrees ? "locatorbar.option.on" : "locatorbar.option.off");
    }

    private void toggleOffscreenArrows() {
        selectedShowOffscreenArrows = !selectedShowOffscreenArrows;
        offscreenArrowsButton.setMessage(offscreenArrowsButtonText());
        LocatorBarConfig.setShowOffscreenArrows(selectedShowOffscreenArrows);
        LocatorBarConfig.save();
    }

    private Component offscreenArrowsButtonText() {
        return Component.translatable(selectedShowOffscreenArrows ? "locatorbar.option.on" : "locatorbar.option.off");
    }

    private void toggleFadeAnimation() {
        selectedFadeAnimation = !selectedFadeAnimation;
        fadeAnimationButton.setMessage(fadeAnimationButtonText());
        LocatorBarConfig.setFadeAnimation(selectedFadeAnimation);
        LocatorBarConfig.save();
    }

    private Component fadeAnimationButtonText() {
        return Component.translatable(selectedFadeAnimation ? "locatorbar.option.on" : "locatorbar.option.off");
    }

    private void toggleShowMarkerDistances() {
        selectedShowMarkerDistances = !selectedShowMarkerDistances;
        showMarkerDistancesButton.setMessage(showMarkerDistancesButtonText());
        LocatorBarConfig.setShowMarkerDistances(selectedShowMarkerDistances);
        LocatorBarConfig.save();
    }

    private Component showMarkerDistancesButtonText() {
        return Component.translatable(selectedShowMarkerDistances ? "locatorbar.option.on" : "locatorbar.option.off");
    }

    private void toggleShowMarkerHeight() {
        selectedShowMarkerHeight = !selectedShowMarkerHeight;
        showMarkerHeightButton.setMessage(showMarkerHeightButtonText());
        LocatorBarConfig.setShowMarkerHeight(selectedShowMarkerHeight);
        LocatorBarConfig.save();
    }

    private Component showMarkerHeightButtonText() {
        return Component.translatable(selectedShowMarkerHeight ? "locatorbar.option.on" : "locatorbar.option.off");
    }
    ...
    private void updateControlStates() {
        ...
        themeButton.active = reworkedStyle;
        dragHudButton.active = styleEnabled;
        ...
        soundPackButton.active = selectedSoundEnabled;
        degreesButton.active = reworkedStyle;
        offscreenArrowsButton.active = styleEnabled;
        ...
        showMarkerDistancesButton.active = styleEnabled;
        showMarkerHeightButton.active = styleEnabled;
        ...
        if (this.list != null) {
            ...
            if (page == 0) {
                ...
                if (selectedStyle == LocatorBarStyle.REWORKED) {
                    this.list.addEntry(Component.translatable("locatorbar.config.field.theme"), themeButton);
                    this.list.addEntry(Component.translatable("locatorbar.config.field.position"), dragHudButton);
                    this.list.addEntry(Component.translatable("locatorbar.config.field.sound"), soundButton);
                    this.list.addEntry(Component.translatable("locatorbar.config.field.sound_pack"), soundPackButton);
                    this.list.addEntry(Component.translatable("locatorbar.config.field.fade_animation"), fadeAnimationButton);
                ...
                this.list.addEntry(Component.translatable("locatorbar.config.field.degrees"), degreesButton);
                ...
                this.list.addEntry(Component.translatable("locatorbar.config.field.marker_distances"), showMarkerDistancesButton);
                this.list.addEntry(Component.translatable("locatorbar.config.field.marker_height"), showMarkerHeightButton);
                this.list.addEntry(Component.translatable("locatorbar.config.field.offscreen_arrows"), offscreenArrowsButton);
            } else {
        ...
        *///?}
        ...
        for (pl.fuzjajadrowa.locatorbar.waypoint.CustomWaypointStore.CustomWaypoint wp : pl.fuzjajadrowa.locatorbar.waypoint.CustomWaypointStore.getForWorld(currentWorld)) {
            if (!seenIds.contains(wp.id())) {
                waypoints.add(new ManagedWaypoint(wp.id(), wp.icon(), wp.color(), wp.visible(), wp.world(), -1));
                seenIds.add(wp.id());
            }
        }
        ...
        *///?}
        ...
        class WaypointEntry extends AbstractEntry {
            ...
            private void deleteWaypoint() {
                ...
                pl.fuzjajadrowa.locatorbar.waypoint.CustomWaypointStore.remove(waypoint.id);
